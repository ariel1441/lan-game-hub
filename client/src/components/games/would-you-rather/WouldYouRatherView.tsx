      ...
      <GameHud
        ...
        rightSlot={(
          <>
            ...
            <GameBadge tone={state.phase === 'revealed' ? 'accent' : 'success'}>
              {state.phase === 'revealed' ? 'Results revealed' : 'Answers hidden'}
            </GameBadge>
          </>
        )}
        bottomSlot={(
          <>
            {canRevealEarly ? (
              <GamePrimaryButton disabled={isRevealing} onClick={handleReveal}>
                {isRevealing ? 'Revealing...' : 'Reveal now'}
              </GamePrimaryButton>
            ) : null}
            {canStartNextRound ? (
              <GamePrimaryButton disabled={isNextRoundLoading} onClick={handleNextRound}>
